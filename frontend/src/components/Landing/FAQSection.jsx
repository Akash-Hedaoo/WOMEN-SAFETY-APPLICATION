...
function FAQItem({ item, isOpen, onToggle }) {
  return (
    <div className={`border-b border-[#8d9d4f]/8 transition-colors duration-300 ${isOpen ? 'bg-[#f5f0e0]/50' : ''}`}>
      <button onClick={onToggle} className="w-full flex items-center justify-between py-6 px-6 text-left">
        <h3 className={`font-headline text-base lg:text-lg font-semibold pr-8 transition-colors duration-300 ${isOpen ? 'text-[#8d9d4f]' : 'text-[#2c2a1e]'}`}>
          {item.question}
        </h3>
        <span className={`material-symbols-outlined text-xl flex-shrink-0 transition-transform duration-300 ${isOpen ? 'rotate-180 text-[#8d9d4f]' : 'text-[#3a3520]/40'}`}>
          keyboard_arrow_down
        </span>
      </button>
      <div className="overflow-hidden transition-all duration-300 ease-in-out" style={{ maxHeight: isOpen ? '200px' : '0px' }}>
        <p className="px-6 pb-6 font-body text-[#3a3520]/60 leading-relaxed">{item.answer}</p>
      </div>
    </div>
  );
}
